...
effectsizegenes <- data.frame(gene = colnames(genelist_brain)[1:(ncol(genelist_brain)-3)], 
                              ...
                              eta_loc = rep(NA, ncol(genelist_brain)-3), 
                              eta_locxeco = rep(NA, ncol(genelist_brain)-3))

### calculate effect size for Location and interaction for each deg, put into table
...
for(i in 1:nrow(effectsizegenes)){
  ...
  effectsizegenes[i,"eta_loc"] <- eta_squared(lm(genelist_brain[,i] ~ecotype+location+location*ecotype, genelist_brain))[2]  #[2,2]
  #effectsizegenes[i,"eta_locxeco"] <- eta_squared(lm(genelist_brain[,i] ~ ecotype+location+location*ecotype,genelist_brain))[3]  #[3,2]
} 
...
effectsizegenes_liver <- data.frame(gene = colnames(genelist_liver)[1:(ncol(genelist_liver)-3)], 
                              eta_loc = rep(NA, ncol(genelist_liver)-3), 
                              eta_eco = rep(NA, ncol(genelist_liver)-3), 
                              eta_locxeco = rep(NA, ncol(genelist_liver)-3))
...
for(i in 1:nrow(effectsizegenes_liver)){
  ...
  effectsizegenes_liver[i,"eta_loc"] <- eta_squared(lm(genelist_liver[,i] ~ecotype+location+location*ecotype, genelist_liver))[2]  #[2,2]
  #effectsizegenes[i,"eta_locxeco"] <- eta_squared(lm(genelist_liver[,i] ~ ecotype+location+location*ecotype,genelist_liver))[3]  #[3,2]
} 
...
effectsizegenes_DSbrain <- data.frame(gene = colnames(genelist_brain_ds)[1:(ncol(genelist_brain_ds)-3)], 
                              eta_loc = rep(NA, ncol(genelist_brain_ds)-3), 
                              eta_eco = rep(NA, ncol(genelist_brain_ds)-3), 
                              eta_locxeco = rep(NA, ncol(genelist_brain_ds)-3))

#effectsizegenes_DSbrain=effectsizegenes_DSbrain[c(2:14285),]

### calculate effect size for Location and interaction for each dsg, put into table
...
for(i in 1:nrow(effectsizegenes_DSbrain)){
  ...
  effectsizegenes_DSbrain[i,"eta_loc"] <- eta_squared(lm(genelist_brain_ds[,i] ~ecotype+location+location*ecotype, genelist_brain_ds))[2]  #[2,2]
  effectsizegenes_DSbrain[i,"eta_locxeco"] <- eta_squared(lm(genelist_brain_ds[,i] ~ ecotype+location+location*ecotype,genelist_brain_ds))[3]  #[3,2]
} 
...
effectsizegenes_DSliver <- data.frame(gene = colnames(genelist_liver_ds)[1:(ncol(genelist_liver_ds)-3)], 
                                      eta_loc = rep(NA, ncol(genelist_liver_ds)-3), 
                                      eta_eco = rep(NA, ncol(genelist_liver_ds)-3), 
                                      eta_locxeco = rep(NA, ncol(genelist_liver_ds)-3))

#effectsizegenes_DSliver=effectsizegenes_DSliver[c(2:14285),]
...
for(i in 1:nrow(effectsizegenes_DSliver)){
  ...
  effectsizegenes_DSliver[i,"eta_loc"] <- eta_squared(lm(genelist_liver_ds[,i] ~ecotype+location+location*ecotype, genelist_liver_ds))[2]  #[2,2]
  effectsizegenes_DSliver[i,"eta_locxeco"] <- eta_squared(lm(genelist_liver_ds[,i] ~ ecotype+location+location*ecotype,genelist_liver_ds))[3]  #[3,2]
} 
...
effectsizegenes_DMC_brain <- data.frame(gene = colnames(DMC_brain)[1:(ncol(DMC_brain)-3)], 
                                        eta_loc = rep(NA, ncol(DMC_brain)-3), 
                                        eta_eco = rep(NA, ncol(DMC_brain)-3), 
                                        eta_locxeco = rep(NA, ncol(DMC_brain)-3))


### calculate effect size for Location and interaction for each dmc, put into table
...
for(i in 1:nrow(effectsizegenes_DMC_brain)){
  ...
  effectsizegenes_DMC_brain[i,"eta_loc"] <- eta_squared(lm(DMC_brain[,i] ~ecotype+location+location*ecotype, DMC_brain))[2]  #[2,2]
  effectsizegenes_DMC_brain[i,"eta_locxeco"] <- eta_squared(lm(DMC_brain[,i] ~ ecotype+location+location*ecotype,DMC_brain))[3]  #[3,2]
} 
...
effectsizegenes_DMC_liver <- data.frame(gene = colnames(DMC_liver)[1:(ncol(DMC_liver)-3)], 
                                        eta_loc = rep(NA, ncol(DMC_liver)-3), 
                                        eta_eco = rep(NA, ncol(DMC_liver)-3), 
                                        eta_locxeco = rep(NA, ncol(DMC_liver)-3))
...
for(i in 1:nrow(effectsizegenes_DMC_liver)){
  ...
  effectsizegenes_DMC_liver[i,"eta_loc"] <- eta_squared(lm(DMC_liver[,i] ~ecotype+location+location*ecotype, DMC_liver))[2]  #[2,2]
  effectsizegenes_DMC_liver[i,"eta_locxeco"] <- eta_squared(lm(DMC_liver[,i] ~ ecotype+location+location*ecotype,DMC_liver))[3]  #[3,2]
} 
...
DEmiR_brain_rlog=read.table("brain_mirna_rlog.csv",header = T,sep = ",",check.names = F,row.names = 1)
  ...
  effectsizegenes_DEmiR_brain <- data.frame(gene = colnames(DEmiR_brain)[1:(ncol(DEmiR_brain)-3)], 
                                            eta_loc = rep(NA, ncol(DEmiR_brain)-3), 
                                            eta_eco = rep(NA, ncol(DEmiR_brain)-3), 
                                            eta_locxeco = rep(NA, ncol(DEmiR_brain)-3))
  

  
  ### calculate effect size for Location and interaction for each DEmiR, put into table
  ...
  for(i in 1:nrow(effectsizegenes_DEmiR_brain)){
    ...
    effectsizegenes_DEmiR_brain[i,"eta_loc"] <- eta_squared(lm(DEmiR_brain[,i] ~ecotype+location+location*ecotype, DEmiR_brain))[2]  #[2,2]
    effectsizegenes_DEmiR_brain[i,"eta_locxeco"] <- eta_squared(lm(DEmiR_brain[,i] ~ ecotype+location+location*ecotype,DEmiR_brain))[3]  #[3,2]
  } 
...
effectsizegenes_DEmiR_liver <- data.frame(gene = colnames(DEmiR_liver)[1:(ncol(DEmiR_liver)-3)], 
                                          eta_loc = rep(NA, ncol(DEmiR_liver)-3), 
                                          eta_eco = rep(NA, ncol(DEmiR_liver)-3), 
                                          eta_locxeco = rep(NA, ncol(DEmiR_liver)-3))
...
for(i in 1:nrow(effectsizegenes_DEmiR_liver)){
  ...
  effectsizegenes_DEmiR_liver[i,"eta_loc"] <- eta_squared(lm(DEmiR_liver[,i] ~ecotype+location+location*ecotype, DEmiR_liver))[2]  #[2,2]
  effectsizegenes_DEmiR_liver[i,"eta_locxeco"] <- eta_squared(lm(DEmiR_liver[,i] ~ ecotype+location+location*ecotype,DEmiR_liver))[3]  #[3,2]
} 
